...
#include "velaros_ros2_qemu_init.h"
...
static int parse_message_count(int argc, char * argv[])
{
  ...
  if (end == argv[1] || *end != '\0' || count < 1 || count > 1000) {
    fprintf(stderr, "usage: velaros_ros2_listener [1..1000]\n");
    return -1;
  }
...
int main(int argc, char * argv[])
{
  ...
  if (message_count < 0 || type_support == NULL) {
    return 1;
  }
  ...
  if (velaros_configure_qemu_interop(&init_options) != RCL_RET_OK) {
