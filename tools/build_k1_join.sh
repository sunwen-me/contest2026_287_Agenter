...
#
# Build and package the RTL8852BS station join diagnostic image for the
# MUSE Pi Pro K1.  The board configuration, the CMake output directory and the
# U-Boot package directory are the ones every station join board run uses,
# so an incremental build keeps its ccache and its object tree.
#
# Usage: build_k1_join.sh [--clean] [--jobs N]
...
CONFIG_PATH="${CONTEST_ROOT}/board/k1/muse_pi_pro/configs/wireless_join_diag"
BUILD_DIR="${WORKSPACE_ROOT}/cmake_out/k1-join"
PACKAGE_DIR="${WORKSPACE_ROOT}/out/k1-join"
