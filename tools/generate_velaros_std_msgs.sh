...
#
# Generate the target-side std_msgs/msg/String C and Fast RTPS C sources.
...
PACKAGE_ROOT="${SOURCE_ROOT}/common_interfaces/std_msgs"
...
Options:
  --check       Verify the generated std_msgs/String source set
  --force       Regenerate even when the input fingerprint is unchanged
  -h, --help    Show this help

Only source and header files for std_msgs/msg/String are exported. Host ROS
objects, archives, shared libraries, and Python modules never enter openvela.
EOF
}
...
INPUT_SHA256="$(
  {
    sha256sum "${PACKAGE_ROOT}/msg/String.msg"
    printf '%s\n' \
      "${COMMON_INTERFACES_REV}" \
...
validate_output()
{
  ...
  [[ -f "${root}/generation.manifest" ]] ||
    fail "std_msgs generation manifest is missing"
  grep -qx "INPUT_SHA256=${INPUT_SHA256}" "${root}/generation.manifest" ||
    fail "generated std_msgs/String sources are stale"

  for relative_path in "${GENERATED_FILES[@]}"; do
    [[ -f "${root}/${relative_path}" ]] ||
      fail "generated std_msgs file is missing: ${relative_path}"
  done
  ...
  ((${generated_count} == ${#GENERATED_FILES[@]})) ||
    fail "expected ${#GENERATED_FILES[@]} std_msgs files, found ${generated_count}"

  if grep -R -l -E '/opt/ros|\.so([.0-9]*)?$' \
      "${root}"/rosidl_* >/dev/null; then
    fail "host ROS path or shared-library reference found in std_msgs source"
  fi

  printf 'Generated std_msgs/String interfaces: PASS (%s files)\n' \
    "${generated_count}"
...
ln -s "${PACKAGE_ROOT}" "${WORK_ROOT}/src/std_msgs"
