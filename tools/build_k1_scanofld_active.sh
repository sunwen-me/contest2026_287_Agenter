...
#
# Build and package the RTL8852BS active scan-offload diagnostic image for the
# MUSE Pi Pro K1.  The board configuration, the CMake output directory and the
# U-Boot package directory are the ones every active-scan board run has used,
# so an incremental build keeps its ccache and its object tree.
#
# Usage: build_k1_scanofld_active.sh [--clean] [--jobs N]
...
CONFIG_PATH="${CONTEST_ROOT}/board/k1/muse_pi_pro/configs/wireless_scanofld_active_diag"
BUILD_DIR="${WORKSPACE_ROOT}/cmake_out/k1-scanofld-active"
PACKAGE_DIR="${WORKSPACE_ROOT}/out/k1-scanofld-active"
