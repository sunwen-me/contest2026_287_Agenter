...
### 增量 3r：把刚发出去的那一帧从 MAC 里绕回来，直接看载荷是密文还是明文

3q 把问题压到了一句话：**硬件到底把明文加密了没有。** 靠 AP 已经问不出来——ACK
在解密之前发出，所以「明文数据帧 → 照样 ACK → 密码层静默丢弃」与现有全部观测
一致。要区分，只能不经过空口、不经过 AP，自己看一眼发出去的那一帧。

#### 为什么 MAC 回环能回答这个问题

回环开关是 CMAC 侧寄存器（`0xcc20`，`B_AX_MACLBK_EN`），而安全引擎是 DMAC 侧的
块（`R_AX_SEC_ENG_CTRL`，本移植读到 `after=0x8000273f`）。也就是说回环的抽头在
加密之后：一帧绕回主机时，引擎该做的事已经做完了。于是同一帧回到 RX 路径上，有
三个互相独立的证人：

1. **帧体。** LLC/SNAP 之后的部分正是引擎会替换成密文的那一段。回来的帧体如果与
   提交的明文逐字节相同，就说明没有任何东西加密过它。
2. **长度。** CCMP 在尾部追加 8 字节完整性校验码，所以加密过的帧回来时比提交时长
   8 字节（68 → 76）。
3. **`hw_dec`。** 回程也有自己的安全引擎，被回程解密过的帧读起来和从未加密过的帧
   一模一样，所以这一位必须比帧体比较更优先——否则会把「回程解开了」误报成
   「发送端根本没加密」。

**CCMP 头不是证人。** 那 8 字节头是本组件自己写进去的，引擎跑没跑它都在；PN 在往
返中存活只说明头没被改写。所以判定只看上面三样。

增量 3n 的 maclbk 探针（run 52）已经证明这条通路是活的：置上 `B_AX_MACLBK_EN`
后计数器绕回 13 个 PPDU（`count-after-hi=0xd`），主机 RX 也确实收到 13 帧
`A2 = 自己`（`self-tx=0xd self-preq=0xd self-tx-a1=ff:ff:ff:ff:ff:ff`）。

#### 五个读数，先在内存里判一遍

判定器 `k1_rtl8852bs_runtime_loopback_classify()` 是个纯函数，编号而不是命名，
因为这行日志既给人读也给判据的正则读：

| 判定 | 值 | 含义 |
| --- | --- | --- |
| `NONE` | `0x0` | 没分类到帧 |
| `CIPHER` | `0x1` | 帧体变了：发送端确实加密 |
| `HEADER` | `0x2` | 追加了 8 字节但帧体没变：两个证人互相不解释，单独给一个判定 |
| `PLAIN` | `0x3` | 就是提交的明文——**本增量要找的就是这一个** |
| `RXDEC` | `0x4` | 回程解密过，载荷说明不了发送端的事 |
| `SHORT` | `0x5` | 短得读不到任何一个偏移 |

`k1_rtl8852bs_runtime_loopback_selftest()` 把这五个读数在内存里喂给它一遍，比较
用的明文由提交时的同一个构帧器 `arp_probe_build` 生成，所以测的是判定器本身而不是
一份手抄的帧。这条自检行是本增量唯一的硬判据（判据总数 **38 → 39**）：

```
K1 Wi-Fi GPL: resident loopback selftest plain=0x3 cipher=0x1 header=0x2
  rxdec=0x4 short=0x5 diff=0x24 first=0x0 llc=0x8 pn=0x1 stage=0x0 status=0x0
```

（实际输出每个值都是 `0x` ＋ 16 位十六进制；`diff=0x24` 是 68−24−8＝36 字节密文跨度。）

#### 回读本身故意不设判据

`k1_rtl8852bs_runtime_secure_loopback_readback()` 存下 `0xcc20`、清计数器、置上
`MACLBK_EN` 并回读确认，提交**一帧**带 `TXRPT_TAG_LOOPBACK`（`0x5`）的受保护
ARP，然后最多轮询 300 ms 找 `A2 = 自己` 的数据帧，判定，无条件恢复 `0xcc20`。
帧的 A3 用广播、tpa 用网关，所以这一级不需要从空口学到任何对端，任何窗口里都能跑。

它排在窗口收尾之后、判决计算之前：回环会把电台从正在运行的窗口底下抽走，而且它的
判定是关于发送路径的读数，不是关于这个窗口的——**回来一帧明文是结论，不是这次运行
失败**。所以判据只要求这行在、能解析，不要求 verdict 取任何值，正则也把
`verdict`/`llc`/`diff-bytes`/`hw-dec` 解出来打印成人话。

```
K1 Wi-Fi GPL: resident loopback readback verdict=… looped=… count-clr=…
  reads=… self=… frame=… len=… fc=… seq=… hdr=… llc=… diff-bytes=…
  diff-first=… pn=… prot=… hw-dec=… sw-dec=… a1-match=… icv=… sec-type=…
  sec-cam=… body=… tx=… stage=… status=…
```

`body=` 打的是 802.11 头结束处的 8 字节：引擎跑了就是 CCMP 头的 PN/KeyID，没跑就是
LLC/SNAP 头的开头，两者都不是密钥材料也不是密文。**密文跨度本身只报计数
...
### 增量 3t：把 A1 换成自己——在回环里问接收路径

3s 把 (A) 判掉之后，剩下两支，其中 (B2) 是可以自己判的：**同一个回环窗口里再发一帧，只把 A1
从 BSSID 换成自己的 MAC**。这一帧的形状就是 AP 发给本站的那种帧——FC 说 from-DS，A1 是自己，
A2 是 AP，A3 也是 AP——除此以外和第一帧一模一样：同一条 pairwise TK、同一个 SEC CAM 表项、
同样的 24＋8 头部、同样 36 字节的 LLC/SNAP ＋ ARP 正文。

#### 为什么这一帧能回答问题

两个方向的 CCMP nonce 都取「手上这一帧的 A2」，所以发送引擎和接收引擎在这里对得上，跟它们在
空口上对得上是同一个理由；密钥两个方向也是同一条成对表项。于是三种读数各对应一个结论：

- 回来的帧解开了（`a1-match=1 hw-dec=1 icv=0 sec-cam=0`）：(B2) 排除，剩下的只有 (B1)，
  也就是 AP 不转发，那一支要主机权限去有线段看。
- 回来的帧 `a1-match=0`：ADDR_CAM 里本机地址那一栏就是问题所在，而它**一个人**就能解释
  「发得出去、被 ACK、没人回」。
- `a1-match=1` 但没解开，或者解开时命中的是 `sec-cam=1`（GTK）：ADDR_CAM 的密钥映射问题——
  发送方向能命中成对表项、接收方向不能。

MAC 回环期间空口进不来，所以 FIFO 里满足「A1 ＝自己 且 A2 ＝BSSID」的帧就是这一帧。万一真有
一帧从空口进来了，它带的也是这两个地址、回答的也是同一个问题，`pn=` 会把两者分开（本帧的 CCMP
头是本组件自己写的）。

#### 代码上只动了一处：帧构造器多一个方向开关

`k1_rtl8852bs_runtime_arp_probe_build()` 加了一个 `downlink` 参数：`false` 走原来的 to-DS
（A1 ＝BSSID、A2 ＝自己），`true` 走 from-DS（A1 ＝自己、A2 ＝BSSID），FC 相应从 `0x4108`
换成 `0x4208`。其余八个调用点全部传 `false`，行为一个字节都没变。

这个开关本身在**离线**诊断里先查过：`runtime protected data TX` 那条现在多打一个 `dl-fc=`，
而且在打印之前就核对过 from-DS 形态的 FC、A1、A2、A3 和长度，不对就 `-EIO` 带行号退出——错在
这里只花一次编译，不花一次上板。核对完再把 to-DS 形态建回去，因为那才是那个诊断接着要发的帧。

#### 读数：另起一行，第一行的字段一个都不动

回读报告现在打两行。第一行还是 3r/3s 的那一行（讲发送路径），第二行是新的（讲接收路径）：

```
resident loopback downlink sent= reads= seen= frame= len= fc= seq= hdr= llc=
  diff-bytes= diff-first= verdict= pn= prot= hw-dec= sw-dec= a1-match= icv=
  sec-type= sec-cam= body= tx= stage= status=
```

`seen=` 是「A1 ＝自己 且 A2 ＝BSSID」的帧数，其余字段与第一行同名同义。这一帧复用 3r 的分类
器，所以 `verdict=0x4`（RXDEC）＝接收引擎解密了它，`llc=0x8 diff-bytes=0x0` ＝解出来的正文与
提交的明文逐字节相同。第二帧的结果放在**自己**的结构体里（堆上再要一个，理由与 3s 的工作区
相同：常驻窗口里栈是稀缺资源），不会把一个已经读到第一帧的窗口变成失败的窗口——一帧 from-DS
帧回不来，本来就是它要取的两个读数之一。窗口在发第二帧之前就退出时，那一行会打
`sent=0x0 status=0xb`（EAGAIN）而不是一行零，免得「没跑」被读成「收到一帧全零」。
...
### 增量 3u：从已经在收的 Beacon 里读 TIM——问 AP 手里有没有留给我们的帧

3t 把 (B2) 排除之后，剩下的 (B1) 是「AP 到底有没有产生那个回复」。这件事本移植原来无从判
断：日志上「发出去、被 ACK、没人回」和「AP 回了但我们没收下」长得一模一样，两种情况的计数
器读数完全相同。

TIM（Traffic Indication Map，IEEE 802.11 clause 9.4.2.6）恰好是 Beacon 里唯一一个专门讲
「这一站」的字段。AP 为某个 AID 缓存了单播帧时，会把位图里该 AID 的位置 1；这一位是 AP 自
己填的，不是本移植推断的。run 58 的关联响应给出 `aid=0x1`，所以要读的就是位图第 0 字节的第
1 位。

#### 这一增量一个字节都不发

它挂在已经在跑的常驻窗口的 Beacon 分支上：`beacons_target` 每加一次，就顺手把那一帧的元素
列表走一遍。没有新的 H2C、没有新的发送、没有新的寄存器写入，所以它不可能改变被测系统的行
为——这是把它选作下一步的主要原因，前面几个增量都得先发一帧才能问出东西来。

#### 位图算术：bitmap control 的偏移是以两字节为单位的

标准把 TIM 拆成三个固定字节（DTIM count、DTIM period、bitmap control）加一段部分虚拟位图。
bitmap control 的第 0 位是「有组播/广播帧待发」，第 1..7 位是**位图偏移，单位是两字节**：

```
offset_bytes = (control >> 1) * 2
byte_index   = aid / 8
bit          = aid % 8
```

要读的字节在元素体里的下标是 `byte_index - offset_bytes`，并且必须先确认它落在
`bmap-len = length - 3` 之内，否则就是这一帧没有携带本站那个字节。越界的情况只记一个计数，
不去读元素外面的内存。

一个容易读错的地方：**元素没有携带的字节按定义就是 0**——clause 9.4.2.6 允许省掉首尾的全零
八位组，省掉的那些位就表示「没有缓存」。所以 `out-of-range` 不是「读不出来」，而是「隐含地
读成 0」；它单独计数只是为了把显式的零和隐含的零分开看，结论上和 `aid-set=0` 站在同一边。
最小合法形态（`length=4`，位图一个字节且为 0）表示除组播位以外全零，这时偏移是 0，本站的字
节正好在范围内。

#### 判据不加：40 条不变

`resident window tim` 行挂在既有的 `--require-runtime-resident` 里，只要求这一行存在，不要
求其中任何一个值——因为两种读数都是结论，不是「好」和「坏」。判据总数仍然是 40。

#### 读数怎么用

驱动多打一行（实际是一行，这里为了排版折了）：

```
K1 Wi-Fi GPL: resident window tim aid= seen= aid-set= bcast-set= absent=
  short= out-of-range= dtim-count= dtim-period= ctl= bmap-len= head=
```

`head=` 是第一份 TIM 元素体的前 8 字节（DTIM count、period、control，然后是位图的头几个字
节）；窗口里没读到 TIM 时 `head=` 后面什么都不打。四种读数各自的意思：

- `aid=0x0`：关联响应的 AID 没拿到，位图里没有一位属于本站——读数作废，不是阴性。
- `seen=0x0`：窗口里没有一帧目标 Beacon 带 TIM——这个窗口什么也没说。
- `aid-set` 非零：**AP 手里正压着一帧给本站。** 那么回复是产生过的、从来没被取走，缺的东西
  在本移植这一侧——AP 认为这一站在省电睡眠。接下来该做的是省电状态机（把自己声明成醒着，或
  者发 PS-Poll 把缓存帧取回来），而不是继续查发送路径。
- `aid-set=0x0` 而 `seen` 非零：这些 Beacon 没有一份把本站的位置 1，AP 没有为本站缓存任何
  东西——回复根本没有产生，要问的是 AP 自己的转发（客户端隔离，或者要求先有 DHCP 地址），
  这一半只能在 AP 的有线段上抓包确认。
...
### 增量 3v：AP 有没有把本站发上去的广播帧又播回来——这一侧唯一能证明上行通了的读数

3u 把「答案压在 AP 队列里、AP 以为本站在睡眠」这条支路划掉之后，剩下两种可能：「上行根本没被
AP 转发」和「AP 回了但本端没收下」。分开它们看上去只能在 AP 的有线段抓包，但还有一种读数既不
需要 AP 配合、也不需要再多发一个字节：**桥转发的定义本身。**

网桥把一帧组播/广播泛洪到除入端口以外的所有端口；一个 BSS 在桥上算一个端口，但同一个 BSS 里还
挂着别的 STA，所以 AP 必须把某个 STA 发上来的广播帧再从空口播出去一遍，否则同一个 AP 下的两个
无线客户端之间 ARP／mDNS 全都不通。mac80211 的 AP 模式就是这么做的：`ieee80211_rx_h_data` 对组
播帧克隆一份 `xmit_skb`，再从无线接口发出去。**原发送者自己也会收到这一份**——广播没法把原发送
者排除掉。

于是判据很干净：空口上只要出现一帧「AP 发的（FromDS=1、ToDS=0）、A3（也就是载荷的源地址）＝本
站 MAC」的帧，它只能是 AP 把本站刚发上去的广播帧收下、解密、又播了出来。这一帧是 AP 自己的字
节，不是从 TX report 推断的，所以它一次性证明三件事：**上行帧被 AP 收到了、用成对密钥解开了、
并且被转发了。** 到那一步，缺的东西就一定在下行。

反过来不成立，这一点写进了代码注释和判据解读：不把组播帧播回原 BSS 的 AP 读数同样是 0，所以 0
不是「上行没通」的证明，只是「这一行什么也没说」。

实现上这是一个极小的增量。常驻窗口对每一帧都已经解析了三个地址，所以只是在 `self_frames`
（A2 ＝ 本站，硬件回环那一份）后面多加一个判断：零新发送、零新 H2C、零寄存器写入，和 3u 一样不
可能改变被测行为。窗口本来就会发一帧广播 ARP Request（`attempt & 1` 的那一次）和一帧广播 DHCP
Discover，所以有东西可播。新增一行：

```
resident window echo frames= group= data= fc= len= head=
```
...
### 增量 3x：一次成功不是一个速率——同一台 peer、窗内六次 ARP、逐次记账

run 63 拿到了本端历史上第一次 `replies=0x1`：AP 用本站的成对密钥把一帧 ARP
Reply 单播回来，硬件解密无完整性错误，驱动解出的正是本站问过的那个地址。但那
一行只说明「往返能发生过一次」，不能说明它可重复。在它之前的四次运行
（`053802Z`/run 62、`044828Z`、`194727Z` 问网关 192.168.1.2，`050954Z` 问一台
普通主机 192.168.1.152）全部是 `replies=0x0`，能解释这个差别的候选有三个，而
一次运行分不开它们：

1. **错误包放行位。** 3w 在窗内置了 `ce20` 的 bit 13 `B_AX_A_ERR_PKT`，让安全
   引擎验不过的帧也交上来。这一支本来就弱——run 63 那帧的读数是
   `icv=0 crc=0`，RMAC 根本没把它归成错误包，所以这个位在那次里没起作用。
2. **问的是哪台主机。** run 63 问的是 `192.168.1.144`（一台普通主机），前面几
   次多数问网关。但 `050954Z` 问的 `.152` 也不是网关，一样没有回答，所以这一
   支也弱。
3. **重传运气。** run 63 的 `ccxrpt-tag-fail=0x1`：四个带报告标签的发送里有一
   个没被确认。窗内只发两次请求时，丢一次就等于丢一半——这一支只能用「发多
   次、数回答」来量。

所以 3x 不加新的空口行为，只把同一件事做够次数并逐次记账：

- **窗长 3000 → 5000 ms**，ARP 首次延迟仍是 1100 ms，**间隔 800 → 650 ms，
  次数 2 → 6**。六次的最后一次落在 1100 + 5×650 = 4350 ms，它的回答还在窗内。
- **锁定 peer。** `k1_rtl8852bs_runtime_resident_arp_tx()` 第一次发送时把学到
  的 peer（先 `arp_peer_*`，否则 `ipv4_peer_*`）抄进 `arp_trip_mac/ip` 并置
  `arp_trip_valid`，以后每次都问这一台。六次问同一台，才是一个速率；六次问不
  同的主机，只是六个互不可比的单次。
- **两种投递形式各半。** 偶数次把以太网目的地址写成学到的那台站（报告标签
  `TAG_ARP` = 3），奇数次写广播（`TAG_ARP_BROADCAST` = 4）。两者的 A1 都是
  AP，所以两者都走成对密钥保护，区别只在分发系统被要求怎么转发。
- **逐次记账。** `arp_trip_credit()` 在每轮轮询开头和窗退出后各调一次：只要
  `arp_replies` 比上次记下的值涨了，就给「最后一次发送」的那一位记一分，一次
  发送最多记一分，并按奇偶累加 `uni-ack`/`bcast-ack`。发送侧只在
  `ret >= 0`（队列收下了）时才计入 `attempts`，所以速率的分母是真的发出去的
  次数。
- **去掉 `count.arp_replies == 0` 这个停止条件**——3w 里它让第一次成功之后不
  再发；要速率就必须发完六次。

窗尾多一行：

```
K1 Wi-Fi GPL: resident window arp trip attempts= replies= mask= uni= uni-ack= bcast= bcast-ack= peer= ip=
```

`mask` 一位一次尝试，bit 0 是第一次。harness 里这一行是**只要求存在**、不要求
任何数值的检查，验收数仍是 40；它把 `replies/attempts` 读成速率，并分别打印
「全中」「部分中（这正是重传运气的样子）」「全不中（那么 run 63 的回答不是这
...
### 增量 4c：报告要说真话，同一帧不要做两遍

run 68 留下两件事：一件是报表撒了谎，一件是标准要求做而本移植一直没做的过滤。4c 把两件都
修完，`91eb838` 和 `d256230`。

**第一件：`dst=` 印的不是这一发真正的目的地。** `icmp_echo_target`／`icmp_echo_alt`／
...
更新（写进去的是 `target_mac`，这一发的目的 MAC）；逐次打印改用 `icmp_echo_last_ip`。判
...
```
resident window dup checked= retries= dropped= streams= evictions= last-seq= last-tid=
```

判据脚本新增 `resident_dup_result` 一块与一条 `missing` 项，`arp_model_result` 的自检正
则同步换成二十段的期望值（`dup=0x6 dup-retries=0x5 dup-dropped=0x2`、
`dup-streams=0x3 dup-evictions=0x0`）——和代码在同一个提交里改，这是 run 67 的教训。
...
```
resident window dup checked=0x1f retries=0x1 dropped=0x1 streams=0x1 evictions=0x0 last-seq=0x60 last-tid=0x10
```

31 帧过了过滤器，其中 1 帧带 Retry 位，1 帧被丢在动作之前。`streams=0x1` 是只缓存过一条
流（AP 的 addr2 加 TID 16——下行是普通 Data 而不是 QoS Data，
`data a1 first fc=0x4208`），`evictions=0x0` 因此让 `dropped=0x1` 成为精确总数而不是下
界。`last-tid=0x10` 就是那个表示「非 QoS」的 16，`last-seq=0x60` 右移四位是序号 6。

**这一帧不是 ping 的重传。** 这一窗 `icmp serve requests=0x0 sent=0x0`——主机侧的 ping 没
活到窗内（原因见下一节），窗内没有任何外部 echo 请求进来。被丢掉的是 AP 自己重传的一帧下
...
自检二十段全过：`dup=0x6 dup-retries=0x5 dup-dropped=0x2`、
`dup-streams=0x3 dup-evictions=0x0 stage=0x0`。

### 工具：主机侧 `ping` 为什么活不到窗内——地址还没进 neigh 表，iputils 自己就退了

run 69 和 70 的主机 ping 都在两三秒后就结束了，`-w 900` 完全没起作用：

```
6 packets transmitted, 0 received, +5 errors, 100% packet loss, time 2772ms
pipe 5
```
...
所以修法不是 `setsid`，是退出就重开：新增 `tools/k1_host_ping.sh`，在截止时间之前循环重
...
运行 74 从头到尾 PASS，日志 `out/k1-serial/k1-wpa-20260901T182629Z.log`（645073 字节），
上一节给增量 5a 定的判据一条不缺：

```
RF RCK path=0x0 save=0x11 mode=0x33c00 sts=0x9cc08
        polls=0x0 done=0x1 trig=0x4e40 code=0x13 rck=0x4c13
RF RCK path=0x1 save=0x11 mode=0x33c00 sts=0x94808
        polls=0x0 done=0x1 trig=0x4a40 code=0x12 rck=0x4812
```

`done=0x1`——RF 0x1c 的 BIT(3) 置起来了，这是三十份日志以来第一次；`code=` 是 0x13 与
0x12，五位以内的小数而不是 0；`rck=` 的低五位正是那个 code（0x4c13 & 0x1f == 0x13，
0x4812 & 0x1f == 0x12），说明写回那一步也生效了。`sts=0x9cc08` 与 `0x94808` 的 BIT(3) 都
是 1。`polls=0x0` 两条路径都是——这条总线上一次读远远超过 1 µs，等第一次读回来的时候 done
位早就置好了，所以轮询一次都没进。

日志里 26 条 `RF RCK path` 行，路径 0 在不同批次里给出 0x11／0x12／0x13 三个码（对应
`rck=0x4411`／`0x4812`／`0x4c13`），路径 1 每次都是 0x12。同一颗芯片在不同批次读出相邻的
校准码是 RC 校准正常的抖动，不是错。

采样侧的变化更直白：`scan RF readback before` 与 `after` 一共 52 次采样，读到的是
`rck=0x4411 rck-sts=0x8c408`（2 次）、`rck=0x4812 rck-sts=0x94808`（44 次）、
`rck=0x4c13 rck-sts=0x9cc08`（6 次），**没有一次是老的 `rck=0x3a00 rck-sts=0x73800`**。
整份日志里 `0x3a00` 只剩两条，都在 553／555 行的 `pre-si-reset` 上——那是第一轮初始化、
RCK 还没跑的时候采的，而 RCK 落在串行接口复位之后，所以这两条本来就该是默认值。后面 24
条 `pre-si-reset` 读的全是校准值。

数据面没有被打扰：`--require-*` 每一道闸都过了，扫描、认证、关联、四次握手、单播与广播数
据全都照旧，最后 `PASS: K1 wireless RAM image reached NSH`。增量 5a 就此在硬件上闭环。

### 增量 5b-1：数模校准的开场——前端字、转换器复位、D die RC 校准

原厂 `halrf_dack_8852b()`（`halrf_dack_8852b.c:909`）的顺序是固定的十一步：
`afe_init → dack_reset → drck` 、`RF A/B 0x5=0`、`RF A/B 0x0=0x337e1`、`addck`、
`addck_reload`、`RF A/B 0x1=0`、`dack(s0,s1)`、`dack_dump`、`dack_done=true`、
`RF A/B 0x5=1`。这个增量只做前三步，因为前三步不碰射频、只碰基带的模拟前端，可以在接收支
路活着的情况下先验一遍。

`halrf_afe_init_8852b()` 先把 MAC 的 PHY 寄存器写周期字压成"全部一个周期"，再给两条路径
各写五个前端字（`0x4486888c`、`0xc6ba10e0`、`0x30c52868`、`0x05008128`、`0x0000272b`），
顺序照抄。移植在每条路径的首字上前后各采一次，日志因此能说清射频镜像有没有已经把这些值放
好。

`halrf_dack_reset_8852b()` 是四个转换器控制字各拉低再拉高。原厂在校准即将开始时脉冲它
们、从不读回，移植也不读回。

`halrf_drck_8852b()` 是 D die 的 RC 校准：`0xc0cc` BIT(6) 置 1 触发，轮询 `0xc0d0` 的
BIT(3)，从 `0xc0d0[19:15]` 取码，把码写进 `0xc0cc[4:0]` 并置上 `0xc0cc` 的选择位，最后脉
冲 `0xc094` BIT(9)。轮询按本地总线的实际情况改成 200 次每次 1 µs（原厂 20000 次），超时
只记录不致命——和 5a 里 RCK 的处理一致，done 位和码都进报表，报表自己会说校准有没有出结
果。

### 增量 5b-2：接收侧转换器的直流校准（ADDCK）

这是十一步里的第四到第七步：把两个射频停下来、量两路转换器的直流、校准、把量出来的偏移写
回去，然后把射频放开。发射侧那半（`dack_8852b_s0/s1` 加 MSBK）需要射频在整段里保持停车状
态，还没做，所以这个增量在收尾处一定把射频放开。

`halrf_addck_8852b()` 每条路径的动作序列是对称的：先把**两条**路径的手动位都关掉，也就是
`0xc0f4[5:4]=0` 与 `0xc1f4[5:4]=0`，然后开转换器时钟（`0x12b8`／`0x32b8` BIT(30)），
`0x032c` BIT(30) 清零、BIT(22) 清零再置起，`0x030c[27:24]=0xf` 按住复位，**延时 100 µs
**，`0x032c` BIT(16) 清零，`0xc0d4`／`0xc1d4` BIT(1) 置起把转换器输入短路，
`0x030c[27:24]=0x3` 放开复位，量一次直流，`0xc0f4` BIT(11) 置起再清零触发校准，延时 1 µ
s，`0xc0f4[9:8]=1` 把结果口切到 done，轮询 `0xc0fc`／`0xc1fc` BIT(0)，再量一次直流，然后
按相反顺序还原，最后从 `0xc0fc` 的 [19:10] 与 [9:0] 取出 I／Q 两个码，关掉转换器时钟。

重载（`halrf_addck_reload_8852b()`）把码写回三个字段：`0xc0f0[25:16]` 收 I 码，
`0xc0f4[3:0]` 收 Q 码的高四位，`0xc0f0[31:26]` 收 Q 码的低六位，最后 `0xc0f4[5:4]=3` 把
手动位打开。

直流测量（`halrf_check_addc_8852b()`）把一个调试口指向某一路转换器（`0x20f4` BIT(24) 清
零、`0x20f8` 的 BIT(31) 置起、`0x20f0` 的三组字段设成 source=1／width=2／mode=0，再用
`0x20f0[7:6]` 选路径），把 `0x1730` 读若干次，两个轴各按 12 位补码折平均：大于 `0x800`
的算负数、按 `0x1000` 回绕累加，最后正负两堆相减除以次数。这个平均值只进日志，不做任何输
入。

**三处刻意的偏离，都写在代码注释里。** ADDCK 的轮询是 200 次而不是原厂的 10000 次——这条
SDIO 总线上一次读远不止 1 µs，实际用掉几次也一并打进日志；直流平均取 32 次而不是 100
次，理由同上，而且这个平均值本来就只是一行日志；停车与放开的两个射频字**都**先采样后还
原，而原厂把 RF 0x5 固定写回 0x1、RF 0x0 压根不还原。第三处是从运行 74 的日志里直接读出
来的：板上活着的 RF 0x0 是 `0x3b7e1`（两条路径、串行接口复位之后都是），而原厂 DACK 里写
的是 `0x337e1`，照抄就会在扫描活着的时候把射频重新配一遍。RF 0x1 那一写不属于这个增量——
它在发射侧那半里，等 5b-3。采样时读到 0 的字在放开时跳过，因为 0 是停车值而不是放开值。

日志一共五行：停车行给出 `fifo=`／`rst=` 与两条路径的 `save`／`mode`，每条路径一行给出
`dc=<前 I>/<前 Q>-><后 I>/<后 Q>`、`polls=`、`done=`、`i=`／`q=` 两个码与结果字，重载行
给出两条路径写完之后读回的手动字，放开行给出还原回去的两个 `save`。

编译 exit 0，text 833194 → 838508，多出 5314 字节，警告仍只有两条已知的 `role_cam` 未使
用函数；`out/k1-wpa/contest-nuttx-flat.bin` 的 SHA-256 是
`f88337647f586b99921499e61a0acdd7b4f111af54474fe493b1e1be88698df0`。

下一次上板（运行 75）同时验 5b-1 与 5b-2，要看的是：`RF AFE phyreg=` 那行的前后采样；
`RF DRCK` 行里 `done=1`、`code=` 是个五位以内的数并且回写进了 `trig[4:0]`；两条
`RF ADDCK path=` 行里 `done=1`、`i=`／`q=` 非零、`dc=` 的前后两对数明显动过；
`RF ADDCK set` 行反映写进去的偏移；以及 `RF ADDCK unpark` 的两个 `save` 等于 `park` 行的
那两个（按运行 74 的读数应当是 0x11）。

### 运行 75：前端字、D die RC 校准与接收侧直流校准都在板上跑通

运行 75 从头到尾 PASS，日志 `out/k1-serial/k1-wpa-20260901T191959Z.log`（601991 字节），
`wlan0` 扫到 3 个 BSS，`--require-*` 每道闸都过，ARP claim／serve、ICMP echo、DHCP
request、loopback 与 CCMP 自检全都照旧。新加的两段校准跑了 13 轮，26 条路径行，**没有任
何一行带 `error=`**——每一步在每一轮都返回 OK。

**前端字（5b-1 第一步）。** `RF AFE phyreg=0xe->0xf` 只在第一轮出现，之后 12 轮都是
`0xf->0xf`——PHY 写周期字第一次被压成"全部一个周期"，之后本来就是了。两条路径的首个前端字
在写之前就已经是 `0x4486888c`，写完还是 `0x4486888c`：射频镜像早把原厂那个值放好了，这一
写是幂等的。这正是当初给这条采样定的问题，现在有答案了。

**D die RC 校准（5b-1 第三步）。** 13 轮读数一字不差：

```
RF DRCK sts=0x9cc08 polls=0x0 done=0x1 code=0x13 trig=0x4c13
```

`sts` 的 BIT(3) 置起，`done=0x1`；`code=0x13` 是个五位以内的非零值；`trig=0x4c13` 的低五
位正是 0x13，说明回写生效，而 BIT(9) 是 0，说明结果选择位按原厂的 `RCK_SEL=0` 清掉了。
`polls=0x0`——第一次读回来的时候 done 早就置好了。

有一个值得记下来的巧合：这三个数与运行 74 里**路径 0 的 RCK** 逐位相同（RF 0x1b 读回
`0x4c13`，RF 0x1c 读回 `0x9cc08`，code 同为 0x13）。这不是采样串了：DRCK 之前最后一次射
频访问是路径 1 的 RCK，读的是 `0x4812`／`0x94808`，如果 `0xc0cc`／`0xc0d0` 读到的是上一
次射频读的残留，这里就该是路径 1 那两个数。而且每个字段都对得上原厂给 `0xc0d0`／`0xc0cc`
定的版图：BIT(3) 是 done、[19:15] 是码、[4:0] 收回写、BIT(9) 是选择位。合理的解释是这两
处是同一个 RC 校准硬核的两个窗口——D die 的数字部分本来就在射频那颗芯片上——所以两边读出同
样的码。留作观察，不当缺陷。

**接收侧直流校准（5b-2）。** 26 条路径行全部 `done=0x1`、`polls=0x0`。两条路径的码稳定且
各有偏向：路径 0 是 `i=0x1`（9 轮）或 `0x2`（4 轮）、`q=0xa`（12 轮）或 `0x8`（1 轮）；
路径 1 是 `i=0x3f7`～`0x3fa`、`q=0x3f8`～`0x3fc`——十位有符号字段里的 −7 上下。逐轮抖动只
有 ±1。

真正说明校准干了活的是 `dc=` 那一对。下面是两条真实日志行，只把 16 位宽的十六进制缩掉了
前导零：

```
RF ADDCK path=0x0 dc=0x0/0xff7->0x0/0x0 polls=0x0 done=0x1
        i=0x1 q=0xa res=0x40a
RF ADDCK path=0x1 dc=0x6/0x8->0x0/0x0 polls=0x0 done=0x1
        i=0x3f9 q=0x3f9 res=0xfe7f9
```

校准之前路径 0 的 Q 轴一直落在 −8～−10（`0xff6`～`0xff8`），I 轴本来就在 −2～+1；路径 1
是 I 轴 +5～+8、Q 轴 +3～+9。校准之后 26 条行里 12 条是 `0x0/0x0`，其余是 `0x1/0x0`（5
条）、`0x0/0x1`（3 条）、`0x1/0x1`（2 条）、`0x2/0x0`（2 条），另有两条 −1 的 `0xfff`——
两个轴的残余直流全部落进 2 个计数以内。转换器的直流被校掉了。

`res=` 是结果寄存器 `0xc0fc`／`0xc1fc` 的原始读回值，I 在 [19:10]、Q 在 [9:0]，所以
`res` 恒等于 `(i<<10)|q`：`0x40a` 就是 `0x1`／`0xa`，`0xfe7f9` 就是 `0x3f9`／`0x3f9`。26
条行逐条都对得上，说明 `i=`／`q=` 不是另算出来的，就是这颗芯片自己给出的码。

`RF ADDCK set` 行把写回去的偏移读了回来，字段也对得上。字段名里的 `0x0`／`0x1` 是路径
号，日志把路径号也按十六进制打了，所以看起来是 `s0x0=`／`s0x1=`：`s0x0=0x28010000` 的 [
25:16] 是 `0x001`，即 I 码；[31:26] 是 `0xa`，即 Q 码的低六位。`s0x1=0xe7f90000` 的 [
25:16] 是 `0x3f9`、[31:26] 是 `0x39`，正是 `0x3f9` 的低六位。i 读到 `0x3f7`、q 读到
`0x3fc` 那一轮写的是 `0xf3f70000`，[31:26] 是 `0x3c`，同样是那一轮 Q 的低六位——每轮的
`set` 都跟着那轮的码走，不是一个写死的常数。13 轮
`RF ADDCK unpark save0x0=0x11 save0x1=0x11` 与 `park` 行的 `save` 完全一致，两个射频都放
开了。`rst=0xc0c0c0c` 的 [27:24] 是 0xc，正是收尾时要还原回去的那个值。

**那处刻意的偏离在板上被证明是必要的。** 13 轮 `park` 行里有 2 轮读到路径 0 的
`mode0x0=0x3b7e1` 而不是 `0x337e1`。原厂对 RF 0x0 只写不还原，照抄的话这两轮就会把射频留
在被重新配过的状态下，而这两轮的扫描是活着的。采样后还原不是纸上的保险，是真的救了两轮。

于是原厂 DACK 十一步里的前七步在硬件上闭环。剩下的是发射侧那半：`halrf_dack_8852b_s0`／
`_s1`（每条路径十七步、两个 MSBK 与一个 DADCK 的轮询）、`halrf_dack_backup_s0/s1_8852b`
（每条路径 16 项 MSBK 加 biasK 与 DADCK）、`halrf_dack_reload_8852b` 以及
`halrf_dack_dump_8852b`，另外 `RF 0x1=0` 那一写属于这一段。原厂两条路径的寄存器是 +0x100
的镜像（`0xc004`→`0xc104`、`0xc040`→`0xc140`、`0xc05c`→`0xc15c` 等），但 s0 的 MSBK 轮询
用 `||`（等两个都置起）而 s1 用 `&&`（等任意一个置起），DADCK 那两处同样不一致——移植时统
一按 s0 的写法等两个，并在注释里记下原厂的这处不对称。

### 运行 76：发射侧转换器校准在板上收敛，原厂那处「固定收尾」被证明不是还原

增量 5b-3 把 `halrf_dack_8852b()` 连同两条路径各自的 backup 搬上板。13 轮 bring-up 各跑
一遍，`RF DACK` 五种日志行一共 130 条，没有任何一条带 `error=`。行的形状是（日志里每个值
都写满 16 位十六进制，这里为了读把前导零去掉）：

```
RF DACK park fifo=0xe041e000 rst=0xc0c0c0c save=0x11/0x11
        mode=0x337e1/0x337e1 opt=0x0/0x0
RF DACK path=0x0 mpolls=0x28/0x0 mdone=0x1/0x1 dpolls=0x0/0x0 ddone=0x1/0x1
RF DACK code path=0x0 bias=0x39/0x2e dadck=0x7c/0x91
        gate=0x24903056->0x24907056
RF DACK msbk path=0x0 side=0x0 w=0x8b91918f/0x898d837d/0x817f817b/0x796f7373
```

**四个等待全部等到。** 两条路径、13 轮、每轮两侧的 MSBK 与 DADCK，`mdone` 与 `ddone` 一
律是 `0x1/0x1`，一次超时都没有。`mpolls=0x28/0x0` 的形状值得记一笔：第 9 步同时给 I、Q
两侧开自动模式，等到 I 侧的 done 位需要 0x28 = 40 次 1 微秒轮询，而那时 Q 侧的 done 位早
就置起了，所以 Q 侧读到 0 次。原厂给这两个等待的上限是 10000 微秒，40 微秒离它很远。
`dpolls=0x0/0x0` 说明第 12 步之后那 1 微秒的延时已经够 DADCK 收敛。

**16 个高位权重是真的在测。** 52 条 `msbk` 行（13 轮 × 2 路径 × 2 侧）彼此全不相同，没有
重复，也没有全零或全 0xff 的饱和。数值都落在 0x80 附近，即每段权重都被修到满量程的一半左
右，这正是分段 DAC 权重校准该有的样子。路径 1 的 I 侧最能看出趋势：
`0xb0b6aea8/0x9a9a968e/0x90847e76/0x70665c50` 展开是 b0 b6 ae a8 9a 9a 96 8e 90 84 7e 76
70 66 5c 50，一条从 0xb0 单调压到 0x50 的斜坡。

**bias 与 DADCK 稳定且两侧不同。** 路径 0 的 `bias` 是 0x39～0x3a / 0x2e，路径 1 是 0x3a
～0x3b / 0x30；`dadck` 路径 0 是 I 0x7a～0x7f、Q 0x8f～0x93，路径 1 是 I 0x8e～0x91、Q
0x7a～0x7d。都围着 8 位码的中点 0x80 摆动几个计数，两条路径的 I、Q 恰好互换——是器件本身
的不对称，不是移植的错。

**原厂那处固定收尾不是还原。** 这次特意在开头和收尾各读一次前端 gate，就是为了回答这个问
题。第 16 步往 `0x12a0`／`0x32a0` 的 [14:12] 写死 0x7，而两条路径第一轮读到的都是
`gate=0x24903056->0x24907056`：进来时 [14:12] 是 0x3，出去时是 0x7。此后 12 轮全是
`0x24907056->0x24907056`。也就是说 BB 初始化镜像留下的是 0x3，原厂 DACK 第一次跑就把它永
久改成 0x7 并且从不改回去——这不是还原，是一次单向的改动，原厂自己的稳态也是 0x7。既然它
之后的扫描、四次握手和数据面全都照旧通过，这里就不需要再加第四处偏离，只需要把它记下来：
谁以后看到 [14:12] 从 0x3 变成 0x7，那是 DACK 干的，是预期行为。

**那两处刻意的还原被下一轮的采样直接证明。** DACK 第 2 步把 `0x030c` 的 BIT(28) 置起、把
`0x032c` 的 BIT(31) 清掉，原厂全树没有任何地方还原它们。板上的证据是：13 轮
`RF ADDCK park` 读到的 `rst` 一律是 `0xc0c0c0c`（BIT(28) 是 0），`fifo` 的最高位一律是
1。如果 DACK 漏了这两处还原，第二轮起的 ADDCK 采样就会读到 `0x1c0c0c0c` 和
`0x6041e000`。换句话说，还原写对了，而且是下一轮自己说的。

顺带看清了一件先前没注意的事：`RF ADDCK park` 的 `fifo` 只有第一轮是 `0xe000e000`，其余
12 轮都是 `0xe041e000`。多出来的 BIT(22) 与 BIT(16) 是第一轮 ADDCK 自己的收尾写进去的，
跟 DACK 无关；DACK 的还原只碰 BIT(31)，其余位一位没动，所以 13 轮 `RF DACK park` 的
`fifo` 完全一致。

`opt=0x0/0x0` 也要记一笔：这块板上 RF 0x1 本来就是 0，所以 park 的那一写是空操作，收尾时
「采样为零就跳过」的规则跳过的也正是一次会写出同一个值的写。规则本身没被这块板检验到，但
它在这里不会错——RF 0x1 的 park 值就是 0。

数据面没有任何变化：被动扫描仍然报 5 个 BSS（其中 3 个带 SSID）、四次握手 `complete=0x1`
且 `gtk-len=0x10`、CCMP 自测五项全 1、`loopback readback verdict=0x1`，40 条验收要求全
过，收尾打出 `PASS: K1 wireless RAM image reached NSH`。

到这里 `halrf_dac_cal_8852b()` 的十一步全部在硬件上闭环，转换器校准这一段做完了。原厂注
释掉的 `halrf_dack_reload_8852b` 不移植（校准自己就把答案写进了工作寄存器，reload 是为电
源状态切换后重建准备的），`halrf_check_dadc_8852b` 也不移植（原厂只在路径 B 上调、且纯粹
为它自己打日志）。优先级 2 剩下的是 `halrf_get_efuse_trim`（热敏、PA 偏置与 TSSI 修
正）、`halrf_rfk_self_init`、按信道跑的那组 `halrf_chl_rfk_trigger`（RX DCK／IQK／DPK／
TXGAPK）、`set_enable_bb_rf(hal, 0)`、五张 `init_rf_reg` 存储表以及 halbb 的
`phy_reg_gain`。

### 增量 5c-1：出厂修正是 eFuse 里的四个格子加一张表，不是一次测量

`halrf_dm_init()` 在 DACK 之后立刻跑 `halrf_get_efuse_trim(rf, HW_PHY_0)`，原厂注释写的
是 `/*Thermal Trim, PA Bias k, TSSI Trim get efuse and set reg*/`。这个函数不在
`halrf_8852b.c` 里，在 `halrf_kfree_8852b.c`——Realtek 把出厂校准这一族叫 kfree。缓存里本
来没有这个文件，是照 `vendor-paths.txt:828` 的路径按固定版本单独取下来的。

读完之后结论是：**这一步没有任何测量**。它只做三件事——把隐藏 eFuse 窗口里的四个格子读出
来、换算成寄存器码、写两个射频寄存器；再把十六个格子读成一张 TSSI 表存着，这一步不写寄存
器。

隐藏窗口是**物理地址**，不是逻辑偏移：`0x5a0..0x5df`，共 0x40 字节。四个修正格子是热敏 A
`0x5df`／B `0x5dc`、PA 偏置 A `0x5de`／B `0x5db`。TSSI 的十六个格子按 2GL、2GH、5GL1、
5GL2、5GM1、5GM2、5GH1、5GH2 排，**地址随频段升高而降低**（路径 A 从 `0x5d6` 递减到
`0x5cf`，路径 B 从 `0x5ab` 递减到 `0x5a4`），所以只能用表，不能用基址加下标。

热敏那次换算差点看错：它是**四位循环右移一位**，`v = ((v & 1) << 3) | (v >> 1)`，掉出低
位的那一位回到最高位。它不是移位，也不是同一个文件里 `_halrf_efuse_exchange_8852b()` 做
的高低半字节交换——那个函数在 8852B 上根本没被调用，所以没有移植。

两处「没烧过」的规则都是**按一对路径判断，不是按单路径**：两个热敏格子都读到 0xff 就完全
不写热敏；两个 PA 偏置格子都是 0xff 就不写偏置；十六个 TSSI 格子**全部**是 0xff 才把整张
表清零，而不是填十六个 -1。

**一处被迫的偏离。** 原厂在修正这一步现场调 `halrf_phy_efuse_get_info()` 去读隐藏窗口，
本移植做不到：驱动里 `k1_rtl8852bs_fwdl_rf_context_diagnostic()` 自己的注释早就写明「它
必须在固件下载之前跑：eFuse 读序列会驱动断电与隔离寄存器，而主机侧间接寄存器窗口一旦
WCPU 接管芯片就不再返回实时值」。所以这 64 字节是从射频上下文阶段本来就要读的物理镜像里
抄一份出来，修正函数从这份副本工作。同样的字节、少一次 eFuse 读、换了个时刻。

第二处偏离不影响功能：原厂在两次写之间套了 `halrf_write_fwofld_start()/_end()`，那是把写
批量交给固件的优化，本移植每次射频写都直接走串行接口，没有可批的东西。

### 增量 5c-2：校准自初始化几乎全是软件状态，唯一碰芯片的是预失真回退放哪

`halrf_rfk_self_init()` 排在噪声控制镜像与 `halrf_si_reset()` 之间，本移植一直没跑过。读
完 `halrf_init.c:275` 以及它调的三个 init，结论是这一步**绝大部分不碰芯片**：逐信道校准
位图清成全 1，第一个请求校准的信道必然拿到一次；TXGAPK 与 IQK 标记为从未跑过；接收直流跟
踪器打开；两条通道退出发射功率跟踪模式；预失真与它的跟踪器使能但禁止重载旧结果；每通道发
射增益上限 `0x3f`；两条通道的接收基带带宽停在 `0xff`——任何真实带宽都不等于它，所以第一个
真实带宽必定被应用。

原厂在这里清零的逐信道预失真备份表**没有移植**：填这张表的阶段还没移植，搬过来就是没人读
的死重量。`halrf_rx_dck_init()` 与 `halrf_tssi_init()` 在 8852B 上没有硬件部分——前者那次
寄存器读属于 8852C／D／BP 分支，这颗芯片走 default。

真正碰芯片的只有 `halrf_dpk_init_8852b()`，它就是 `halrf_set_dpd_backoff_8852b()`：读基
带 `0x44a0`，把 `[16:12]` 的 OFDM 回退加上 `[6:0]` 的标尺，两者之和 ≥ 44 就把预失真回退
搬进基带——把 `0x81bc` 的三个逐通道增益标尺写成 `0x7f7f7f`、并把预失真增益记成 `0x7f`；否
则留给预失真自己处理、记成 `0x5b`。`halrf_kpath_8852b()` 在双频并发拆分关闭时返回两条通
道，本移植从不打开它，所以两条都覆盖。两条分支都在 `0x81bc` 前后各采一次，所以**没写寄存
器的那一轮也留下了没写的证据**。

位置上它排在 pre-si-reset 那次射频采样之后：它一个射频寄存器都不写，采样测到的东西不变，
而相对 `halrf_si_reset()` 的原厂顺序保持不动。

### 运行 78：这块板真的有出厂修正数据，两个增量一次按键同时闭环

先说一件我预测错了的事。**我原以为隐藏窗口是空的**，依据是我记成了这块板的 eFuse 基本没
烧过。这个依据本身也是错的：本文档前面「RF context」那一段早就写着
`tssi_de[0x210..0x259] programmed=0x2d/0x4a`、
`rx_gain_k[0x2d4..0x2dd] programmed=0x5/0xa`——七十四个字节里烧了四十五个、十个里烧了五
个，从来不是空的。隐藏窗口的答案同样是烧过的。

```
RF context hidden[0x5a0..0x5df] programmed=0x17/0x40
        thermal_trim=0x22/0x28 pabias_trim=0x20/0x20
RF TRIM efuse thermal=0x22/0x28 pabias=0x20/0x20 tssi-blank=0x0/0x10
RF TRIM code thermal=0x1/0x4 pa2g=0x0/0x0 pa5g=0x2/0x2 wrote=0x1/0x1
RF TRIM path=0x0 ther=0x5000->0x15000 pab=0x11000->0x20000
        tssi=0x0d000c0f/0x0d0d0e10
RF TRIM path=0x1 ther=0x5000->0x45000 pab=0x11000->0x20000
        tssi=0x0200f4f7/0xfbf5f6f7
```

64 字节里烧了 0x17 = 23 个，四个修正格子一个都不空，十六个 TSSI 格子也**全部**烧过：
`tssi-blank=0x0/0x10`，十六个里一个空的都没有。
也就是说这块板出厂时做过完整的射频标定，只是本移植到现在都没用过它。

**换算与写入被硬件逐位确认。** 热敏 A 读到 0x22，低四位 0x2 = 0b0010 循环右移一位得
0b0001 = `0x1`；B 读到 0x28，0x8 = 0b1000 得 0b0100 = `0x4`——正是日志里的
`code thermal=0x1/0x4`。PA 偏置 0x20 拆成低四位 0x0 给 2 GHz、高四位 0x2 给 5 GHz，也就
是 `pa2g=0x0/0x0 pa5g=0x2/0x2`。

写进去的值可以直接从前后采样里验：射频 `0x43` 掩码 `0x000f0000`，路径 0 从 `0x5000` 变成
`0x15000`（该域 0→1），路径 1 变成 `0x45000`（0→4），与两个码一致。射频 `0x60` 两条路径
都从 `0x11000` 变成 `0x20000`：2 GHz 域 `0x0000f000` 从 1 变成 0，5 GHz 域 `0x000f0000`
从 1 变成 2，同样与两个码一致。**四次写全部落地，一个 `error=` 都没有。** 换算读错的话，
落进寄存器的会是另一个数，所以循环右移这个读法是被板子确认的，不是推断的。

**修正是幂等的。** bring-up 里射频序列跑了多轮，从第二轮起每一行都是
`ther=0x15000->0x15000`、`pab=0x20000->0x20000`：修正写下去就留在那里，重复跑不会叠加，
也不会被中间的校准抹掉。

**TSSI 表的形状是可信的。** 这一步不写寄存器，只把表存下来给以后的 TSSI 用，但数值本身说
明它不是噪声：路径 A 是 `0d 00 0c 0f 0d 0d 0e 10`，路径 B 是 `02 00 f4 f7 fb f5 f6 f7`。
按有符号 8 位读，路径 A 的 5 GHz 六段是 +12～+16，路径 B 是 −12～−5，两条路径在 5 GHz 上
方向相反、量级相当——这正是逐频段功率偏移该有的样子。两条路径的 2GH 都是 0x00，这一段没有
偏移；它们不是空格子，空格子会读成 0xff。

自初始化那边的结果同样干净，而且它那个判断在这块板上落在**要搬**的一侧。

```
RF SELF-INIT chlk-map=0xffffffff gapk/iqk=0x0/0x0 rxdck-track=0x1
        tssi-mode=0x0/0x0 rxbb-bw=0xff/0xff
RF SELF-INIT dpk en/trk/rld=0x1/0x1/0x0 txagc-max=0x3f/0x3f gs=0x7f
RF SELF-INIT dpd scale=0x02b2c3a6 ofdm-bkof=0xc tx-scale=0x26 limit=0x2c
        moved=0x1 backoff=0x0->0x7f7f7f/0x0->0x7f7f7f
```

基带 `0x44a0` 读到 `0x02b2c3a6`：`[16:12]` 是 0xc = 12，`[6:0]` 是 0x26 = 38，合起来
50，过了原厂那个 44 的门槛，所以 `moved=0x1`——预失真回退搬进基带，`0x81bc` 从 `0x0` 写成
`0x7f7f7f`，两条通道都写上了，预失真增益记成 `0x7f` 而不是 `0x5b`。从第二轮起也是
`0x7f7f7f->0x7f7f7f`，同样幂等。

**这一行值得记住**：以后 DPK 移植上来的时候，它该用的增益设定不是默认的 `0x5b`，而是这块
板上算出来的 `0x7f`，因为回退已经在基带里给掉了。如果那时按默认值走，发射会比原厂低一大
截。

数据面这一轮完全照旧：被动扫描报 4 个 BSS（`data-only=1`，这一轮环境里比之前少了一个
AP，跟改动无关）、四次握手 `complete=0x1` 且 `gtk-len=0x10`、CCMP 自测两个向量都复现、
DHCP `ack=0x1` 拿到地址、ICMP 四发四回 `bad=0x0`、`loopback readback verdict=0x1`，40 条
验收要求全过，收尾打出 `PASS: K1 wireless RAM image reached NSH`。

到这里射频初始化里「跑一次就完」的那一段全部闭环：RC 校准、串行接口复位、数模校准十一
步、出厂修正、校准自初始化。优先级 2 剩下的是紧跟修正的 `halrf_tssi_get_efuse_ex`（TSSI
初始化，它要用刚存下来的这张表）、按信道跑的那组 `halrf_chl_rfk_trigger`（RX DCK／IQK／
DPK／TXGAPK）、`set_enable_bb_rf(hal, 0)`、五张 `init_rf_reg` 存储表，以及 halbb 的
`phy_reg_gain`。


### 增量 5c-3：发射功率的 de-emphasis 是五十个 eFuse 格子，这一步只读不写

`halrf_dm_init()` 在出厂修正的下一行就跑 `halrf_tssi_get_efuse_ex()`，这颗芯片上是
`halrf_tssi_get_efuse_8852b()`。这个文件缓存里也没有，照 `vendor-paths.txt:838` 的路径按
固定版本取下来的。

它读**五十个逻辑 eFuse 格子**，每条路径二十五个：六个给 CCK，十九个给调制速率，十九个里
五个是 2 GHz、十四个是 5 GHz。每个格子是一个有符号字节，是某个信道组的功率 de-emphasis。
**这一步一个寄存器都不写**，原厂也不写。

地址来自 `halrf_efuse_8852b.h` 的地址枚举，而这里有个坑：**枚举是连续的，eFuse 不是**。
原厂那四个循环写成 `for (i = FIRST; i <= LAST; i++)`，i 走的是枚举值，每个枚举值再经一个
switch 换成地址；路径 A 的调制格子从 `0x21a` 直接跳到 `0x222`，路径 B 从 `0x244` 跳到
`0x24c`。所以和修正表一样只能用地址表，不能用基址加下标。五十个格子落在 `0x210..0x259`
这七十四字节里，其中二十四个字节不属于任何格子。这五十个地址我逐个跟头文件对过，没有一个
错、没有重复。

判定规则跟着一起搬，一字不改：`halrf_tssi_check_efuse_data_8852b()` 会把 get 再跑一遍，
数有多少格子的低字节是 0xff，**只有五十个全空才判整张表不可用**——四十九个空加一个烧过的
就算可用，那四十九个还是原样保留 0xff，不会被填成别的值。原厂这个 check 在缓存的子集里没
有任何调用者，调用方在 halrf 之上，所以本移植只报结论、不拿它分支。

**偏离和 5c-1 是同一处，同样是被迫的**：逻辑 eFuse 映射在固件下载之前解出来、也在那里释
放，所以这七十四字节和隐藏窗口一起缓存进射频上下文结构体，读格子的函数从副本工作。

这两个增量为什么在原厂里只隔一行，现在能说清楚了：**它们是同一个加法的两个加数**。发射功
率阶段拿当前信道组的 DE 格子，加上那个频段的 TSSI 修正，两者之和写进 CCK 与 OFDM 的
de-emphasis 寄存器。那个阶段还没移植，所以现在读出来、报出来、不动寄存器。

**这一次结果可以事先推出来。** 已有日志里这个窗口 `programmed=0x2d/0x4a`，七十四字节烧了
四十五个；不属于任何格子的只有二十四个字节，所以四十五个里至少有二十一个落在格子上，空格
子最多二十九个，离五十个差得远。也就是说 `usable=0x1` 是算出来的，不是猜的——上一轮我预测
错了，这一轮不需要预测。


### 增量 5c-4：`halrf_dm_init()` 的第二行，发射功率参考的四个基带寄存器

这一步是补的，不是新的。`halrf_dm_init()` 第二行就是 `halrf_set_ref_power_to_struct()`
（`halrf.c:3963`），这颗芯片上是 `halrf_set_ref_power_to_struct_8852b()`
（`halrf_set_pwr_table_8852b.c:1039`）。本移植此前从 `halrf_dm_init()` 里挑出来的都是靠
后的校准，这一行在最前面，一直没跑。

它自己的函数体只有四组赋值：参考偏移的两半清零；零分贝码字 `base_cw_0db` 在射频前端类型
不超过五十时是 `0x27`、超过是 `0x21`；代表十六 dBm 的码字 `tssi_16dBm_cw` 是 `0x12c`；
OFDM 和 CCK 两个参考功率都是 0 dBm。然后它调 `halrf_bb_set_tx_pow_ref()`，那个才碰芯片，
实体是 `halbb_set_tx_pow_ref_8852b()`（`halbb_8852b_api.c:2306`）。

基带那半边每个调制方式一条式子：功率乘二，加参考偏移，再加零分贝码字乘八，得到一个 10.3
格式的有符号数；它的高六位是射频码字，被夹在十五到六十三之间，也就是正负二十四 dBm，低三
位的基带小数照原样留着；发射功率检测偏移是十六 dBm 码字加上功率乘二再减一百二十八。三个
字段按检测偏移在第十八位、码字在第九位、有符号功率在低九位打包，二十七位的结果写到两条路
径的同一个地址：OFDM 是 `0x5804` 和 `0x7804`，CCK 是 `0x5808` 和 `0x7808`。

**这块板的数值可以先算出来。** 日志里 `RF context rfe[0x2ca]=0x1`，射频前端类型是一，不
超过五十，所以 `base_cw_0db=0x27`。参考功率是零、偏移是零，于是 10.3 值是
`39*8=312=0x138`，高六位是三十九，落在十五到六十三之间，不夹；检测偏移是
`300+0-128=172=0xac`；打包得 `0xac<<18 | 0x138<<9 = 0x2b27000`。CCK 那半边参数一样，所以
两个字都是 `0x2b27000`，码字都是 `0x138`。**四个寄存器写同一个值**，这是这块板的 rfe 决
定的，不是巧合。

**旧值要一起报出来，是有原因的。** 这四个寄存器本移植从来没写过，所以它们一直是基带镜像
留下的值，而此前所有发出去的帧——包括跑通的关联和四次握手——都是按那个值发的。这一步是第一
次按原厂的参考去覆盖它，所以每个寄存器都是先读、再写、再读回，`before->after` 一起打印。
如果关联在这一轮退化，唯一的嫌疑就是这四个字，回退也只回退这四个字。

**`halrf_dm_init()` 到这里就查完了。** 剩下那几步在这颗芯片上都不欠：`halrf_fcs_init()`
（`halrf_api.c:249`）整个函数体只在 `chip_id == CHIP_WIFI6_8852A` 时调一次 8852A 的实
现，8852B 走空；`halrf_rfe_type_gpio_setting()` 落到 `halrf_set_gpio()`
（`halrf.c:6198`），里面只有 8852A／8852C／8842A／8192XB／8851B 的分支，没有 8852B；
`halrf_rfe_init()`（`halrf_init.c:627`）只有 8852A 和 8852C 两个 case，8852B 落
`default: break`，不设任何前端标志；`halrf_set_final_rfability()` 是纯软件的一句
`rf->support_ability = dev_cap.rfk_cap & hw_rf_ability`；`halrf_cmd_parser_init()` 是调
试命令行的参数解析器。AACK 和 LCK 早就确认过不在这颗芯片上跑。也就是说 `halrf_dm_init()`
里会碰硬件的每一步，现在要么已移植、要么已证明是空的。

优先级 2 还剩四件事：按信道跑的那组 `halrf_chl_rfk_trigger`（RX DCK／IQK／DPK／
TXGAPK）、`set_enable_bb_rf(hal, 0)`、五张 `init_rf_reg` 存储表，以及 halbb 的
