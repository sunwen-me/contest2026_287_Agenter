...
- **上板一次、修了一处偏移、等再上板（增量 3u，判据仍为 40）：从目标 Beacon 里读 TIM，问 AP 手里
  ...
  `aid-set=0x0` 而 `seen` 非零 ⇒ AP 没有为本站缓存任何东西，回复根本没有产生，要问的是 AP 自
  己的转发（客户端隔离，或者要求先有 DHCP 地址），这一半只能在 AP 的有线段上抓包确认。已编译
...
一半已经实现（增量 3u，等上板）：读目标 Beacon 的 TIM，看本站 AID 那一位有没有被 AP 置 1——置
1 说明回复产生过却从未被取走（AP 认为本站在省电睡眠），整窗为 0 说明回复根本没产生。** 另一条
