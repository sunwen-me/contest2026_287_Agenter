...
- **3s 之后剩下的两种可能，其中第二种在本移植自己身上**：AP 能解开、也 ACK 了（3o 的 TX
  ...
  **(B2) 回复真的来了，但本移植收不到发给自己的单播帧。**这一支已经在 **run 58 排除**
  （见下一条 3t）：回环里一帧 A1 ＝自己的受保护数据帧回来时报 `a1-match=0x1 hw-dec=0x1
  icv=0x0 sec-cam=0x0`，MAC 认得自己的地址，安全引擎命中**成对**密钥表项并正确解密。
  在此之前它从未被排除过：`a1-match` 从关联到 run 57 一直是 `0x0`，接收侧成立的证据全是
  **组播**——run 39 的 2 帧、run 42 的 13 帧、run 55 的 7 帧，`group=` 与 `total=` 相等、命中的
  是 **GTK 那条 CAM 表项（`cam=0x1`）**。限界要一起记住：回环不过 BB/RF，被排除的是 MAC/SEC
  这一层；「空口来的**单播数据帧**」这个组合至今仍未被观察到（空口侧的证据分别是不加密的管理帧
  和 GTK 广播帧）。所以 **(B1) 是现在唯一还站得住的解释**，而它要主机权限：在 AP 的有线段跑
  上面那条 `tcpdump`。
- **已上板并出结论（增量 3t，run 58，40/40，`RUN_EXIT=0`）：回环里第二帧，A1 ＝ 自己 MAC。**
  ...
  **run 58 的读数落在第一种**（`k1-wpa-20260831T194727Z.log`）：
  `sent=0x1 seen=0x1 frame=0x1 len=0x50 fc=0x4208 seq=0xf hdr=0x18 llc=0x8 diff-bytes=0x0
  verdict=0x4 pn=0x1 prot=0x1 hw-dec=0x1 a1-match=0x1 icv=0x0 sec-type=0x6 sec-cam=0x0
  body=06 00 00 20 00 00 00 00 stage=0x0 status=0x0`。`pn=0x1` 说明回来的就是刚发出去的那一帧
  （CCMP PN 相等），`a1-match=0x1` 是关联以来第一次，`sec-cam=0x0` 是成对密钥表项在接收方向上
  第一次被命中，`llc=0x8 diff-bytes=0x0` 说明硬件原地解密、把 8 字节 CCMP 头留在原处而且解出来
  的正文与提交的明文逐字节相同（`body=` 就是那个 CCMP 头：PN 6、ExtIV、KeyID 0，不是密钥也不是
  密文）。同一次的第一行又复现了 3s：`fc=0x4108 diff-bytes=0x24 sw-diff=0x0 sw-mic-diff=0x0`。
  run 57 是这个增量的第一次上板，它在主动扫描处就失败了（`passive scan-offload error=0x3d`
  ＝ENODATA，13 个 PPDU 出去零 Probe Response，bring-up `-61`），属于空接收窗抖动家族，联机代码
  一行未跑；那一次唯一的收获是离线检查给出 `dl-fc=0x4208`。

- **已上板两次、修了一处偏移、读数已出（增量 3u，判据仍为 40）：从目标 Beacon 里读 TIM，问 AP 手里
  有没有留给本站的帧。** 3t 之后剩下的 (B1) 是「AP 到底有没有产生那个回复」，而日志上「发出去、被 ACK、没人
  回」和「AP 回了但本端没收下」读数完全一样。TIM（clause 9.4.2.6）是 Beacon 里唯一一个专门讲
  「这一站」的字段：AP 为某个 AID 缓存了单播帧就把位图里那一位置 1，这一位是 AP 自己填的，不
  是本移植推断的。**这一增量一个字节都不发**——它挂在常驻窗口既有的 Beacon 分支上，
  `beacons_target` 每加一次就顺手走一遍元素列表，没有新的 H2C、没有新的发送、没有新的寄存器
  写入，所以它不可能改变被测系统的行为。位图算术：bitmap control 第 0 位是组播待发，第 1..7
  位是**以两字节为单位的偏移**，所以 `offset_bytes = (ctl >> 1) * 2`、`byte_index = aid / 8`、
  `bit = aid % 8`，读之前先确认 `byte_index - offset_bytes` 落在 `bmap-len = length - 3` 之
  内，越界只记一个计数、不读元素外面的内存。一个容易读错的地方：**元素没有携带的字节按定义就
  是 0**（clause 9.4.2.6 允许省掉首尾的全零八位组），所以 `out-of-range` 不是「读不出来」而是
  「隐含地读成 0」，结论上和 `aid-set=0` 站在同一边，单独计数只为把显式的零和隐含的零分开看。
  新增一行 `resident window tim aid= seen= aid-set= bcast-set= absent= short= out-of-range=
  dtim-count= dtim-period= ctl= bmap-len= head=`（`head=` 是第一份 TIM 元素体的前 8 字节：
  DTIM count、period、control 再加位图头几个字节；窗口里没读到 TIM 时 `head=` 后面什么都不
  打），挂在既有的 `--require-runtime-resident` 里，只要求这一行存在、不要求其中任何一个
  值——两种读数都是结论而不是「好」「坏」，所以判据总数仍是 40。run 58 的关联 AID 是 1，所以要
  读的就是位图第 0 字节的第 1 位。四种读数四个结论：`aid=0x0` ⇒ 关联响应的 AID 没拿到，读数作
  废、不是阴性；`seen=0x0` ⇒ 窗口里没有一帧目标 Beacon 带 TIM，这个窗口什么也没说；
  `aid-set` 非零 ⇒ **AP 手里正压着一帧给本站**，回复产生过却从来没被取走，缺的东西在本端，下
  一步是省电状态机（把自己声明成醒着，或者发 PS-Poll 把缓存帧取回来）而不是继续查发送；
  `aid-set=0x0` 而 `seen` 非零 ⇒ AP 手里没有为本站留着的帧，**这只排掉「答案早就产生了、压在
  AP 的队列里、AP 以为本站在睡眠」这一条支路**（省电／PS-Poll／TIM 轮询都不必做）；反过来不成
  立——AP 认为本站醒着时产生的答案会立刻发出去、根本不会进 TIM，位同样是 0，所以「AP 根本没产
  生答案」和「AP 发了但本端没收下」仍然分不开。已编译
  通过（`text=813036 data=9768 bss=25296`，无新增告警），四条解读分支离线逐个跑过，判据正则对
  四条合成行各命中 13 组。
  **run 59（`k1-wpa-20260901T044828Z.log`，40/40，`RUN_EXIT=0`）的读数是
  `seen=0x0 absent=0x26`：38 帧目标 Beacon 一个 TIM 都没读出来——而 TIM 在 Beacon 里是必备元
  素，所以这不是 AP 的说法，是本移植读错了位置。** 元素表起点当时用了
  `k1_rtl8852bs_runtime_resident_header_length()`，那是给数据帧写的：它按「子类型第 3 位 ＝
  QoS 数据」加两字节，而 **Beacon 子类型正好是 8**，于是起点从 36 变成 38，落进 SSID 元素里
  面，整条链歪掉、越界退出、记一次 `absent`。修正是起点直接写
  `K1_RTL8852BS_IEEE80211_HEADER_SIZE + K1_RTL8852BS_IEEE80211_BEACON_FIXED_SIZE`（管理帧头是
  平的 24 字节，没有 QoS 控制字段也没有第四地址；这和管理帧解析器找 SSID 用的是同一个式子），
  并从签名里去掉 `header_length` 参数。同时新增 `len=`（第一帧目标 Beacon 的实际长度，走元素表
  之前就记），这样下一次能一并排掉「接收路径只交付了帧头」这第二种可能。**一般性的坑：那个
  helper 对所有子类型 ≥ 8 的管理帧都会多加两字节——Beacon(8)、ATIM(9)、Disassoc(10)、
  Auth(11)、Deauth(12)、Action(13)；其余调用点都在数据帧上，所以目前没有别处受影响，但以后读
  认证帧／去关联帧的帧体时必须记住。**
  **run 61（`k1-wpa-20260901T050954Z.log`，`--nsh-reboot` 从 `nsh>` 起、不用按 RST，40 条全过、
  无 `FAIL`）的读数：`aid=0x1 seen=0x25 aid-set=0x0 bcast-set=0x0 absent=0x0 short=0x0
  out-of-range=0x0 dtim-count=0x0 dtim-period=0x1 ctl=0x0 bmap-len=0x1 len=0x196
  head=00010000`——37 帧目标 Beacon 全部读到 TIM、首帧 406 字节，偏移修对了；位图是最小合法形式
  （1 字节、偏移 0、`head=00 01 00 00`），AID 1 那一位 37 次全为 0 且不越界，连组播缓存位也是
  0。按第四条读：AP 手里什么都没压着，省电这条支路划掉，剩下的两种可能仍未分开。同一窗口
  `a1-match=0x0`（13 个数据帧全是组播/广播，AP 从未向本站单播过数据帧）、`arp sent=0x2
  status=0x0 replies=0x0 arp-req=0x5`（别人的广播 ARP 收得到、GTK 解得开，没有人回本站）。
...
- **已上板（增量 3x，run 64）：把 run 63 那次往返做成一个速率——同一台 peer、窗内六次 ARP、逐次
  ...
  3x **不加新的空口行为**，只把同一件事做够次数：窗长 3000 → **5000 ms**，ARP 首延仍 1100 ms，
  间隔 800 → **650 ms**，次数 2 → **6**（最后一次落在 1100 + 5×650 = 4350 ms，回答还在窗内）；
  `runtime_resident_arp_tx()` 第一次发送时把学到的 peer（先 `arp_peer_*`，否则 `ipv4_peer_*`）
  锁进 `arp_trip_mac/ip` 并置 `arp_trip_valid`，**以后每次都问这一台**——六次问同一台才是速率，
  问六台只是六个不可比的单次；偶数次的以太网目的地址写那台站（报告标签 `TAG_ARP`＝3）、奇数次
  写广播（`TAG_ARP_BROADCAST`＝4），两者 A1 都是 AP、都走成对密钥，区别只在分发系统被要求怎么
  转发；新 helper `runtime_resident_arp_trip_credit()` 在每轮轮询开头和窗退出后各调一次，只要
  `arp_replies` 比上次记下的涨了就给「最后一次发送」的那一位记一分（一次发送最多一分，按奇偶累
  加 `uni-ack`/`bcast-ack`），发送侧只在 `ret >= 0` 时才计入 `attempts`，所以速率的分母是真的
  发出去的次数；并**去掉 3w 里 `count.arp_replies == 0` 这个停止条件**（否则第一次成功之后就不
  再发，测不出速率）。窗尾多一行 `resident window arp trip attempts= replies= mask= uni=
  uni-ack= bcast= bcast-ack= peer= ip=`（`mask` 一位一次尝试，bit 0 第一次）。已编译通过
  （`text=815948 data=9768 bss=25296`，SHA256 `b15d79af…`，无新增告警），风格基线不变（驱动超
  79 列 50 行、判据脚本 55 行，零 tab、零行尾空白，`ast.parse` 通过）；harness 里这一行**只要求
...
- **已上板（增量 3y，run 65）：OFFER 不是分配——用受保护的 DHCP Request 把它收下。** RFC 2131
  ...
  `dhcp_offer`/`_attempt`/`_xid`/`server_id`/`server_source`/`mask`/`router`/`lease` 与帧的
  address 3（在分发系统内部发出这一帧的那台站，也就是服务器自己；AP 是 address 2）作
  `dhcp_server_mac`。④窗长 5000 → **7000 ms**，ARP 门后加 Request
...
- **run 65（`out/k1-serial/k1-wpa-20260901T104437Z.log`，0 FAIL，40/40）：DHCP 握手做完了。**
  `resident window dhcp request sent=0x1 status=0x0 sn=0x16 bytes=0x152 reply=0x3 ack=0x1
  ack-ip=0xc0a801ce nak=0x0`——本站在受保护链路上走完 Discover→Offer→Request→Ack，拿到
  **192.168.1.206**，掩码 255.255.255.0、网关 192.168.1.2、租期 43200 秒（`offer` 行：
  `server-id=0xc0a80102 src=0xc0a80102 mask=0xffffff00 router=0xc0a80102 lease=0xa8c0
  offers=0x2 xid=0x8c79b074 mac=60beb40996b8`）。`bytes=0x152`＝338＝326＋12 正是加了选项
  50/54 的长度；`ccxrpt-tag=0x8 ccxrpt-tag-ok=0x8 ccxrpt-tag-fail=0x0`，这一轮连重传都没有。
  **同一个窗里 ARP 仍是 0/6，而被问的那台主机 `60:be:b4:09:96:b8` ＝ 192.168.1.2 正是刚回了
  本站两次 OFFER 一次 ACK 的那台服务器**——同一台主机、同一把密钥、同一个窗、同一条分发路
  径，它显然愿意回答本站，只是没回答那六个请求。于是只剩两个候选：它不回答 SPA＝0 的请求；
  或者它回答了而回复是**单播到本站**的帧，而单播下行在本移植只成功过一次（run 63；本窗
...
- **已上板并出结论（增量 3z，run 66）：把同一台主机再问一遍，这次带上自己的地址。** 除了发送协议地址
  ...
  确认的地址就返回 `-EADDRNOTAVAIL`，claimed 形态**不交替广播**（答案必须单播回本站，用广播
  形态只会把样本砍半），报告标签新增 `TAG_ARP_CLAIM`＝0x7；计数分开
  （`arp_claim_attempts`/`_acks`/`_mask`/`_spa`/`_status`），但**记账仍只有一个 watcher**
  （credit helper 多 `claimed` 参数，窗里用 `arp_trip_last` ＋ `arp_trip_last_claimed` 记住最
  后一次发送属于哪一轮，共享同一个 `arp_trip_seen`——两个 watcher 各自 seen 会把同一个回复记
  两次）；窗 7000 → **9000 ms**，claim 门在 Request 门之后、条件 `dhcp_ack_ip != 0 &&
  arp_trip_valid`，**3** 次、间隔 **700 ms**（ACK 在 run 65 落在窗内约五秒处，三次都装得
  下）。窗尾一行 `resident window arp claim attempts= acks= mask= spa= status= peer= ip=`，
  harness 只要求存在，验收数仍是 **40**，三种解读：命中（SPA＝0 就是探测轮沉默的原因，且回
  复走通了单播下行）／全不中（发送地址不是原因，剩下的只有回答必须走的方向——寻址到本站的
  下行帧）／没发（窗内没拿到被确认的地址）。顺手改掉一处过时解读（ARP 无回答时原来说「下一
...
不是继续猜 ARP，而是把这次握手做完——**增量 3y 已在 run 65 上板并成功：`ack=0x1
...
**增量 4b 已经写完并构建通过（`text=830344 data=9768 bss=25296`，SHA256
`ed5b088300d170632b3768baa4184c400989e3995240991fcf5f305b50044ed3`），run 68 已上板成立。**
4a 只证了往外那一半：本端问、对端答。4b 是第一次本端**欠**一个 IP 应答——一台主机自己决定往
192.168.1.206 发 echo 请求，本端要收下、承认目的地址是自己的、再用它的字段造一帧回去。这也是
整个移植里唯一一处不用读日志就能验的部分：窗口那 12 秒里在同网段任一台机器上 `ping
192.168.1.206`，有回显就成立。RFC 792 要求 identifier／sequence／data 原样带回，所以两种请求是
**明确拒答**而不是尽力答：自己 ICMP 校验和不对的进 `icmp_serve_bad`（答了等于报告一条正在损坏帧
的路是通的），data 超过 64 字节的进 `icmp_serve_long`（截短了答对方照样算往返成功，那是错报）。
四个计数器分开读：`requests=` 只数认下且答得了的，`sent=` 是真发出去的，`requests>0 sent=0`
是「进得来、答不出去」，`requests=0` 是「窗内没人 ping」。构造函数 `..._icmp_echo_build()` 多收
`icmp_type`／`echo_data`／`echo_data_length`，请求和回复共用一条已被证明的路。收包路径不许发帧，
所以观察器只抄下回答需要的字段并置 `icmp_serve_pending`（队列一格深、新的盖旧的、SDIO 写成功才
清），由窗口循环发送；门不计时（对面有人在等，晚到的 echo 回复被任何工具算成丢包）但有上限 8，
是 ARP 那个的两倍，因为 `ping` 一秒一个而窗口 12 秒。`RESIDENT_FRAME_MAX` 128→192 只为让第十四段
在内存里造出那个必须被拒答的超长请求（32+8+20+8+65=133），每一处使用都确认过是诊断缓冲区大小而
不是协议常量；把 64 字节上限压下去不行，Linux `ping` 默认就发 56 字节 data。自检从十一段扩到
十五段：十二段一个发给本站的 ping（id 0x1234、seq 7、40 字节 data）逐字段落对，十三段翻掉它的
ICMP 校验和要求 `bad` 加一且**已排队的那个不被顶掉**，十四段带 65 字节 data 要求 `long` 加一（手工
拼帧，因为本端构造函数拒绝造这种帧，而这正是重点：请求来自外面），十五段在上天线之前逐项检查回复
本身（type 0、id／seq 原样、data 逐字节相同、两地址对调、两个校验和自洽）。新增窗尾一行
`resident window icmp serve requests= sent= bad= long= bytes= peer-ip= id= seq= data= status= mac=`
与逐次的 `resident icmp serve tx …`；判据脚本新增 `resident_icmp_serve_result` 一块与一条
`missing` 项，`arp_model_result` 同步换成十五段的值——**和代码在同一个提交里改，这是 run 67 的
教训**，并且两条正则都先离线验过，验法是从驱动源码按 `k1_early_puts` 字面量顺序把那两行重新拼出来
再匹配（字段顺序与拼写来自代码而不是记忆），同时验了过期的十一段那行会被拒绝。
run 68 的读法：`icmp serve requests=` 非零否 → `sent=`／`status=` → `bad=`／`long=` → 主机侧
`ping` 的输出。它同时复验 `df4c080`（同一镜像重跑 4a 应得全项 PASS）与 4a 留下的两个边界。

**判据可以离线验了：`run_k1_wireless_smoke.py --replay LOG`。** run 67 被判 FAIL 是因为一条过期
正则，改它花几分钟、却花掉了一次上板复位。replay 把捕获日志当成唯一输入，不开设备、不发一个
字节，把全部判据照原样跑一遍；开机那一半抽成了 `boot_from_board()`，判据段一行没动（两种模式
必须是同一份代码，否则 replay 什么也证明不了）。三条要向板子提问的判据（`/dev/ttyHCI0` 打开、
Bluetooth host scan、`wlan0` scan）在 replay 里**明确报成 skipped**，不许读成通过。旗标集照用
`tools/run_k1_wpa.sh --replay <log>`。这次 4b 就是这么验的：run 67 的日志只差两项（4b 那一行
本来就没有 ＋ 十一段自检行匹配不上十五段期望值），其余全过——所以除新增两条外没有别的过期期望；
换 4a 之前的日志差四项，说明它在判别而不是一律放行。**每次改判据先 replay 一遍再上板。**

**run 68 的结果：增量 4b 在实体板上成立，而且是从这台主机外部验的。**
日志 `out/k1-serial/k1-wpa-20260901T135550Z.log`。这台开发主机本身就在 AP 子网上
（`enp6s0 192.168.1.153/24`，网关 192.168.1.2），所以 `ping -D -i 1 -W 1 192.168.1.206`
就能把 4a→4b 整条链路走完，不需要任何人动手：主机侧收到 8 个应答
（`icmp_seq=101..107`，第一个 1097 ms，此后 12～15 ms），`ip neigh` 解出
`192.168.1.206 dev enp6s0 lladdr 84:fc:14:06:79:7b`。板上窗尾：
`icmp serve requests=0xd sent=0x8 bad=0x0 long=0x0 peer-ip=0xc0a80199 id=0x3bf7 seq=0x70
data=0x38 mac=00e2697b46e8` 与 `arp serve requests=0x2 sent=0x2`。**13 个请求、8 个应答，
这个差值本身就是上限生效的证据**（`ECHO_SERVE_MAX=8`，窗口 12 秒、`ping` 一秒一个）；
`data=0x38` 是 56 字节，Linux `ping` 的默认载荷，也就是当初不能把 64 字节上限压下去的原因。
ARP 应答这一窗服务了两台不同的主机（逐次行 `tpa=0xc0a80199` 与 `tpa=0xc0a80102`）。
下行涨到 `data a1 self=0x1b self-prot=0x1b self-dec=0x1b`（27 帧，run 67 是 8 帧），
`icv=0x0 crc=0x0`。自检十五段全过（`stage=0x0 status=0x0`），整体
`PASS: K1 wireless RAM image reached NSH`——run 67 那次 FAIL 闭环。

**run 68 翻掉了 run 67 的第一条保留意见，但暴露了一个报表缺陷（下一个要修的东西）。**
窗尾仍印 `target=0xc0a80102 alt=0xc0a80102`、逐次 `dst=` 也全是网关，看着像交替又折成一台；
实际上 alt 取的是 `arp_reply_*`，而这一窗的 ARP 回复来自这台主机
（`arp reply mac=00e2697b46e8 ip=0xc0a80199`），奇数次尝试确实发给了它——证据是窗尾
`src=0xc0a80199` 配 `seq=0x3`（奇数次走 alt）且 `bad=0x0`。这两个字段之所以错，是因为只
在
`count->icmp_echo_attempts == 0` 时锁存一次，而第一次 echo（`pn=0x3`）比 ARP 轮第一帧
（`pn=0x4`）还早；逐次的 `dst=` 又是从锁存值算的。**改法**：每次尝试都更新
`target=`／`alt=`（或两个候选第一次不相等时重锁），逐次 `dst=` 印当次真正用的地址；
判据脚本那句
「192.168.1.2 and 192.168.1.2 asked」同源，一起改。

**run 68 新增的保留意见：本端没有 802.11 重复帧过滤。** 主机侧 `icmp_seq=101` 收到
`(DUP!)`，板上 `seq=0x65` 答了两次（`requests=0x1`／`0x2` 是两帧请求进来、`pn=0x7`／
`0x8` 是两帧回复出去），主机只发了一次——同一请求被送上来两遍（空口重传 Retry 位，或 AP
交付两遍）。标准要求按 (地址, TID) 缓存上一帧 Sequence Control 并丢掉带 Retry 位的重复帧，
本移植没有这个缓存，于是 13 个请求里有 1 个是重复、8 个应答里有 1 个花在已答过的请求上。
...
站在睡眠」这条支路排除，省电／PS-Poll 不必做；但整窗为 0 并不等于「回复根本没产生」——AP 认为
本站醒着时产生的回复会立刻发出、根本不进 TIM。** 另一条
同样不需要 AP 配合的路子是把探测帧换成一种
**回复必然是广播**的形态（例如 DHCP DISCOVER 把 flags 的 broadcast 位置 1，让 OFFER 以广播回
来），因为「空口 GTK 广播数据帧能收到并解开」是 run 39/42/55 已经成立的唯一一条完整空口接收
证据，而「空口单播数据帧」这个组合至今仍未被观察到。**
**但 DHCP 的 broadcast 位这一形态已经试过（3p/3q，OFFER 从未回来），而消费级 AP／服务器常常无
视这个位仍旧单播 OFFER，所以它不是决定性的。下一个增量应当换成一种应答在协议上本来就是组播的
探测——mDNS 查询（RFC 6762：QU 位清零时响应发往 224.0.0.251 组播）——这样只要上行真的被 AP 解
密并转发到有线段，应答就一定沿着已经证明可用的广播/GTK 路径回来，从而在不要主机权限、也不依
赖那条从未成功过的单播下行路径的前提下判定上行是否真的出去了。**
