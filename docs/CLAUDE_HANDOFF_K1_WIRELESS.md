...
- **已实现、等上板（增量 3u，判据仍为 40）：从目标 Beacon 里读 TIM，问 AP 手里有没有留给本站
  的帧。** 3t 之后剩下的 (B1) 是「AP 到底有没有产生那个回复」，而日志上「发出去、被 ACK、没人
  ...
  通过（`text=812996 data=9768 bss=25296`，无新增告警），四条解读分支离线逐个跑过，判据正则对
  四条合成行各命中 12 组。
